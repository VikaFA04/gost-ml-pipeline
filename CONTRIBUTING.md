...
- `tests/test_profile_quality_acceptance.py` — двухуровневый schema-lint всех профилей в `src/rules/profiles/*.json` (Tier A: все профили; Tier B: только `methodical_guidelines`).
- `tests/test_methodical_extractor.py` — smoke-тесты экстрактора над Бергер PDF фикстурой.

Фикстура коммитится в `tests/fixtures/methodical/normocontrol_berger.pdf` (1.4 MB Феврал 2026; ASCII-имя по соображениям CI-логов). Локальные разработчики могут также прогонять экстрактор против актуальной `normocontrol/Нормоконтроль 2026.pdf` (9.4 MB, gitignored).

Перед PR: `make regression-gate` (6 файлов) или хотя бы `python3 -m pytest -q tests/test_profile_quality_acceptance.py tests/test_methodical_extractor.py`.
